...
export function UpdateCostDisplay(
	...
	if (precision === 0) {
		...
		getElement(elementID).textContent =
			"Cost: " + formatDecimal(variable, precision) + currencyName[currency];
	}
}
...
export const upgrades = {
	...
	mbup: {
		...
		costFunction(upgradeAmount) { //TODO: finish this; jakub is implementing actual upgrade formulas into the upgrade object instead of what i was donig before
			// 100 * 1.5 ^ amount
	...
	bangspeed: {
		...
		costFunction(upgradeAmount) {
			...
			return upgradeAmount
				.min(Decimal.dTwo).pow_base(Decimal.dTwo)
...
export function NBExtra(): void {
	...
	if (getUpgradeTimesBought('unlocknpboost').eq(Decimal.dOne)) {
		...
		getElement('divnp').textContent = "Nuclear Particles: " + formatDecimal(nuclearParticles, 1);
	}

	else {
		getElement('divnp').textContent = "Nuclear Particles: " + formatBig(getUpgradeTimesBought('nuclearbuy'));
	}
}
...
export function NABExtra(): void {
	...
	if (getUpgradeTimesBought('unlocknapboost').eq(Decimal.dOne)) {
		...
		getElement('divnap').textContent = "Nuclear Alpha Particles: " + formatDecimal(nuclearAlphaParticles, 1);
	}

	else {
		getElement('divnap').textContent = "Nuclear Alpha Particles: " + formatBig(getUpgradeTimesBought('nuclearalphabuy'));
	}
}
...
export function AGAExtra(): void {
	if (getUpgradeTimesBought("upgradeaga").lte(4)) {
		...
		player.agaTime = (Decimal.dTen.div(getUpgradeTimesBought("upgradeaga").minus(3))).ceil().toNumber()
	}
}
...
export function MAExtra(): void { //TODO:
	if (getUpgradeTimesBought("upgrademergeautobuyer").lte(4)) {
		player.mergeAutobuyerTime = Math.ceil(player.mergeAutobuyerTime / 2);
	} else {
		player.mergeAutobuyerTime = (Decimal.dTen.div(getUpgradeTimesBought("upgrademergeautobuyer").minus(3))).ceil().toNumber();
	}
}
