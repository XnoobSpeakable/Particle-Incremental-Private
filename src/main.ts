...
function maTestSingle() {
	if (getUpgradeTimesBought('mergeautobuyerunlock').eq(Decimal.dOne)) {
		...
		getElement('divtogglema').textContent = player.mergeAutobuyerToggle ? 'On' : 'Off';
	}
}
...
function amountUpdate() {
	if (getUpgradeTimesBought('unlocknpboost').eq(Decimal.dOne)) {
		...
		getElement('divnp').textContent =
			'Nuclear Particles: ' + formatBig(getUpgradeTimesBought('nuclearbuy'));
	}
	if (getUpgradeTimesBought('unlocknapboost').eq(Decimal.dOne)) {
		...
		getElement('divnap').textContent =
			'Nuclear Alpha Particles: ' + formatBig(getUpgradeTimesBought('nuclearalphabuy'));
	}
	...
	getElement('divobase').textContent = 'You have ' + formatBig(player.omegaBase);
	getElement('omegaalphacost').textContent =
		'Cost: ' + formatBig(player.omegaAlphaCost);
	getElement('divoalpha').textContent = 'You have ' + formatBig(player.omegaAlpha);
...
function loadMisc(): void {
	...
	getElement("counter").innerHTML = "<span style='color: #64ed93;'>" + formatBig(player.num) + "</span> particles";
...
function reactorHandler() {
	reactor.fuelTime = onBought(
		[new Decimal(300), '*', [new Decimal(1.25), '^', 'reactoruptime'], '/', [Decimal.dTwo, '^', 'reactorupmult']]
	)
	...
	if (reactor.isActive) {
		reactor.boost = onBoughtInc(
			[[new Decimal(1.25), '^', 'reactorupmult'], '*', reactor.fuelMult]
		)
		getElement('divreactorstatus').textContent = `Reactor status: Running (${formatDecimal(player.fuel, 2)} Fuel)`
	}
	else {
		reactor.boost = Decimal.dOne;
		getElement('divreactorstatus').textContent = "Reactor status: Out of fuel"
	}
	getElement('divreactorfuelusage').textContent = `When active, your reactor is using up 1 fuel every ${formatBig(reactor.fuelTime.div(10))} seconds`
	...
	getElement('divreactornp').textContent = `Nuclear Particles ${formatBigSpecific(reactor.boost)}x as strong`
	getElement('divreactormb').textContent = `Manual Boost ${formatBigSpecific(totalMBBoost)}x as strong`
	getElement('divreactornap').textContent = `Nuclear Alpha Particles ${formatBigSpecific(NAPfactor)}x as strong`
	getElement('divreactorgb').textContent = `Generator Boost ${formatBigSpecific(GBfactor)}x as strong`
	getElement('divreactorbp').textContent = `Booster Particle gain increase by ${formatBigSpecific(BPfactor)}x`
}
...
window.toggleaga = function (): void {
	if (getUpgradeTimesBought('unlockaga').eq(Decimal.dOne)) {
		...
		getElement('divtoggleaga').textContent =
			player.agaToggle ? "On" : "Off";
	}
};
...
window.buyomegaalpha = function (): void {
	if (player.alphaNum.gte(player.omegaAlphaCost)) {
		...
		getElement('omegaalphacost').textContent =
			'Cost: ' + formatBig(player.omegaAlphaCost);
		getElement('divoalpha').textContent = 'You have ' + formatBigSpecific(player.omegaAlpha);
	}
};
...
window.buyFuel = function (fuelType: Fuels) {
	if (fuelType === 'player.fuel') {
		if (player.num.gte(1e42) && player.alphaNum.gte(1e14) && player.betaNum.gte(50)) {
...
function returnParticleHandler(): void {
	if(getUpgradeTimesBought('buyreturngenerator').gt(0)) {
		const gain = onBought(
			[new Decimal(10), '*', 'buyreturngenerator', '*', player.betaNum, '*', ['rpup', '+', Decimal.dOne]]
		)
		player.returnParticles = player.returnParticles.plus(gain)
	
		alphaFromReturn = onBought(
			[new Decimal('1e9 '), '*', [Decimal.dTwo, '^', 'rpmult'], [totalBoostFromNAP, '+', Decimal.dOne], [Decimal.dTwo, '^', 'alphamachinedouble']]
		)
		// eslint-disable-next-line @typescript-eslint/restrict-template-expressions
		getElement('returnboosttext').textContent = `Your ${formatBig(player.returnParticles)} Return particles (+${formatBig(gain.div(10))}/s) are returning ${formatBig(alphaFromReturn.div(10))} Alpha particles per second`
   }
}
...
function fgbTestConst(): void {
	if (getUpgradeTimesBought('gen').gt(Decimal.dZero)) {
		...
		if (getUpgradeTimesBought('unlocknpboost').eq(Decimal.dOne)) {
			nuclearParticles = onBought(
				['nuclearbuy', '*', [Decimal.dOne, '+', ['upgradenpboost', '+', Decimal.dOne, '/', new Decimal(10)]]]
			)
		...
		if (getUpgradeTimesBought('unlocknapboost').eq(Decimal.dOne)) {
			nuclearAlphaParticles = onBought(
				['nuclearalphabuy', '*', [Decimal.dOne, '+', ['upgradenapboost', '+', Decimal.dOne, '/', new Decimal(10)]]]
			)
      ...
      returnParticleHandler();
		...
		getElement('nptext').textContent = `Nuclear particles add a +${formatDecimal(reactor.boost, 2)}x multiplier to generators, generator boost, and manual boost`
		getElement('naptext').textContent = `Nuclear particles add a +${formatDecimal(reactor.boost, 2)}x multiplier to alpha gain`


		const boostsacmult = new Decimal(1.5).pow(getUpgradeTimesBought('boostsacrifice'))

		getElement('boostsactext').textContent = `Reset your Booster Particles, but increase Booster Particle and Alpha Particle gain. Currently ${formatDecimal(boostsacmult, 1)}x.`
		...
		const betaGain = onBought(
			'betaacc', ['permerge', '+', Decimal.dOne], [new Decimal(2), '^', 'doublebeta'],
		);

		if (player.bangTimeLeft === 0) {
			player.alphaNum = player.alphaNum.plus(alphaGain).plus(alphaFromReturn);
		...
		if (getUpgradeTimesBought('unlockabgb').gt(Decimal.dZero)) {
			abgbBoost = onBoughtInc(
				player.alphaNum.cbrt(), '/', new Decimal(100), '*', 'abgbefficiency', '+', Decimal.dOne
			)
		}

		getElement('abgbtext').textContent = `Your alpha-based generator boost is multiplying your generators by ${formatBig(abgbBoost)}x (cbrt(alpha)/100*${formatBig(getUpgradeTimesBought("abgbefficiency").plus(Decimal.dOne))})`

		const gain: Decimal = onBought(
			['biggerbatches', '+', Decimal.dOne], '*', 'gen', '*', ['speed', '/', new Decimal(10), '+', new Decimal(0.1)], '*', player.genBoostMult, '*', [[totalBoostFromNP, '+', Decimal.dOne], '^', Decimal.dTwo], '*',
			[new Decimal(3), '^', 'threeboost'], '*', [Decimal.dOne, '+', [[player.boosterParticles, '+', Decimal.dOne], '/', new Decimal(100), '*', [['boosteruppercent', '+', Decimal.dOne], '/', new Decimal(100)]]], '*', abgbBoost, "*", GBfactor
		);

		getElement('particlesperclick').textContent =
			'You are getting ' +
			formatBig(onBought(['mbup', '+', Decimal.dOne], '*', ['mbmult', '+', Decimal.dOne], '*', [totalBoostFromNP, '+', Decimal.dOne]).times(clickerParticleMult).times(totalMBBoost)
			) +
			' particles per click';
		...
		getElement('divgbtl').textContent =
			'Boost Time Left: ' + formatBig(player.genBoostTimeLeft.div(Decimal.dTen));
		...
		if (player.boosterParticles.eq(0)) {
			getElement('boostersmaintext').textContent =
				`You are currently getting 0 booster particles per alpha particle per second,
			...
			getElement('boostersmaintext').textContent =
				`You are currently getting ${formatBig(bpGain.times(Decimal.dTen).div(player.alphaNum.max(1)))} booster particles per alpha particle per second,
               resulting in a +${formatBigSpecific(percentBoostDisplay)}% boost to base particle production`;
		}
		else {
			getElement('boostersmaintext').textContent =
				`You are currently getting ${formatBig(bpGain.times(Decimal.dTen).div(player.alphaNum))} booster particles per alpha particle per second,
               resulting in a ${formatBigSpecific(percentBoostDisplay.div(100).plus(1))}x boost to base particle production`;

		}

		getElement('bpamount').textContent =
			'You have ' + formatBig(player.boosterParticles) + ' booster particles';

		const clickerParticleGain = onBought(
			[['machine', '*', [new Decimal(1.5), '^', 'speedparticle']], '/', Decimal.dTen]
		).times(machineProd)
		...
		getElement('omegabasecost').textContent =
			'Cost: ' + formatBig(player.omegaBaseCost);
		getElement('divobase').textContent = 'You have ' + formatDecimal(player.omegaBase, 1);
		getElement('omegaalphacost').textContent =
			'Cost: ' + formatBig(player.omegaAlphaCost);
		getElement('divoalpha').textContent = 'You have ' + formatDecimal(player.omegaAlpha, 2);

		player.num = player.num.plus(gain);

		getElement("particlespersecond").innerHTML =
			"You are getting <span style='color: #ed6464;'>" + formatBig(gain.times(10)) + "</span> particles/s";
		...
		if (player.boosterParticles.gte(1e5) || getUpgradeTimesBought('boostsacrifice').gt(Decimal.dZero)) {
			getElement('bpsacshow').style.display = 'block';
		}

		if (player.betaNum.gte(1e5) || getUpgradeTimesBought('buyreturngenerator').gt(Decimal.dZero)) {
	...
	if (getUpgradeTimesBought('unlockaga').eq(Decimal.dOne)) {
		...
		if (player.agaToggle === true) {
			...
			getElement('untilaga').textContent =
				format(player.groupAutobuyerTimeLeft / 10) + ' left until next autobuy';
		}
	}
}
